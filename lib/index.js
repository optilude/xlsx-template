...
//
    ...
    */
    ...
    Workbook.prototype.substituteImage = function (cell, string, placeholder, substitution, drawing) {
        ...
        var cNvPr = etree.SubElement(nvPicPr, 'xdr:cNvPr', {id: maxId, name: '图片' + maxId, descr: ''});
    ...
    Workbook.prototype.rowHeightToEMUs = function (width) {
        //TODO : need to be verify
        return Math.round(width / 72 * 914400);
    }
            ...
            if(typeof(imageObj) === 'string'  || imageObj instanceof String){
                ...
                //}else{
                    if("imageRootPath" in this.option && fs.existsSync(this.option.imageRootPath + "/" + imageObj)){
                        ...
                        return new Buffer(fs.readFileSync(this.option.imageRootPath + "/" + imageObj, { encoding: 'base64' }), 'base64');
                    }else{
                        if(fs.existsSync(imageObj)){
                            //get the relatif path file
                            return new Buffer(fs.readFileSync(imageObj, { encoding: 'base64' }), 'base64');
                        }
                    }
                //}
                try {
                    var buff = new Buffer(imageObj, 'base64')
                    return buff;
                } catch (error) {
