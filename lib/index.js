    ...
    */
     ...
     */
     Workbook.prototype.substitute = async function(sheetName, substitutions) {
        ...
        var allRows = sheetData.findall("row")
        for(let row of allRows){
            ...
            for(let cell of AllCells){
                ...
                if(cell.attrib.t === "s") {
                    ...
                    var allPlaceHolders = self.extractPlaceholders(string)
                    for(let placeholder of allPlaceHolders){
                        ...
                        if(placeholder.full && placeholder.type === "table" && substitution instanceof Array) {
                            if(placeholder.subType==='image' && drawing == null){
                                if(rels){
                                    drawing = await self.loadDrawing(sheet.root, sheet.filename, rels.root);
                                }else{
                                    console.log("Need to implement initRels. Or init this with Excel")
                                }
                            }
                            newCellsInserted = await self.substituteTable(
                                row, newTableRows,
                                cells, cell,
                            ...
                            if(rels != null){
                                ...
                                string = await self.substituteImage(cell, string, placeholder, substitution, drawing);
                            }else{
                                console.log("Need to implement initRels. Or init this with Excel")
                            }
                        } else {
    ...
    Workbook.prototype.substituteTable = async function(row, newTableRows, cells, cell, namedTables, substitution, key, placeholder, drawing) {
        ...
        if(substitution.length === 0) {
            ...
            for(var idx=0; idx<substitution.length; idx++){
                var element = substitution[idx]
                var newRow, newCell,
                ...
                if(idx === 0) { // insert in the row where the placeholders are
                    ...
                    if(value instanceof Array) {
                        ...
                        await self.substituteImage(cell, placeholder.placeholder, placeholder, value, drawing);
                    } else {
                    ...
                    if(value instanceof Array) {
                        ...
                        await self.substituteImage(newCell, placeholder.placeholder, placeholder, value, drawing);
                    } else {
    ...
    Workbook.prototype.substituteImage = async function (cell, string, placeholder, substitution, drawing) {
        var self = this;
        var self = this;
        ...
        substitution = await self.imageToBuffer(substitution)
        //put image to media.
...
Workbook.prototype.imageToBuffer = async function(imageObj){
        ...
        if(imageObj instanceof Buffer){
            ...
            if(typeof(imageObj) === 'string'  || imageObj instanceof String){
                ...
                if(this.isUrl(imageObj)){
                   var convertedImage =  await imageToBase64(imageObj);
                   imageObj = convertedImage;
                }else{
